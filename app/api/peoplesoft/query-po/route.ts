...
export async function POST(request: NextRequest) {
  try {
    console.log('API Route called: /api/peoplesoft/query-po');
    ...
    // Parse request body
    const { businessUnit, poId } = await request.json();
    console.log('Request params:', { businessUnit, poId });

    if (!businessUnit || !poId) {
      return NextResponse.json(
        { error: "Business Unit and PO ID are required" },
        { status: 400 }
      );
    }
    ...
    const url = `${baseUrl}/X_WS_PO/JSON/NONFILE?isconnectedquery=n&maxrows=999&prompt_uniquepromptname=BU,PO&prompt_fieldvalue=${businessUnit},${poId}&json_resp=true`;

    console.log('Calling PeopleSoft API:', url);
    ...
    console.log('PeopleSoft API response status:', response.status);

    if (!response.ok) {
      const errorText = await response.text();
      console.error('PeopleSoft API error:', errorText);
      return NextResponse.json(
    ...
    // Check if PO was found
    if (!data.data?.query?.rows || data.data.query.rows.length === 0) {
      return NextResponse.json(
        { error: "PO not found in PeopleSoft" },
        { status: 404 }
      );
    }
